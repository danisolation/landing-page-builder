...
- MVP/POC-first product mindset: every claimed feature must actually work end-to-end; dislikes piling on features that cannot run ("ôm đồm nhiều feature quá mà lại chạy không được"). When a feature can't be fully implemented yet (needs an API key, infrastructure, or a product decision), say so explicitly instead of shipping a stub or placebo UI ("nói thẳng thay vì làm giả") — and don't rush half-features that would become the very fake "trust killer" controls the project just cleaned up. The user acts on this by explicitly deferring blocked features until their prerequisites exist (e.g. put AI first-draft on hold until an AI API key is available, custom subdomains until DNS/infrastructure exists) and redirecting effort to implementable polish. Confidence: 0.95
